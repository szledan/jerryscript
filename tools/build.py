...
def get_arguments():
    ...
    devgroup.add_argument('--regexp-compatibility', metavar='X', choices=['ON', 'OFF'], default='ON', type=str.upper,
                          help=devhelp('enable regexp compatibility mode (%(choices)s; default: %(default)s)'))
...
def generate_build_options(arguments):
    ...
    build_options.append('-DFEATURE_REGEXP_COMPATIBILITY=%s' % arguments.regexp_compatibility)
